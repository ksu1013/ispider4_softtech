 ...
 * @author 김승욱
 */
public class CeCNExtension implements Extension, AddonExtension {
    ...
    public void startExtension(DqPageInfo dqPageInfo, String homePath) {
        System.out.println("=== CeCNExtension Start ===");
...
//        BbsPage pageContent = Configuration.getInstance().getBbsPage(dqPageInfo.getBbsId(), 1, "*");
    ...
    public String changeRequestURL(String url, DqPageInfo dqPageInfo) {
        if (url.contains("/?page=")) {
            // URL에서 마지막 '/' 이후의 문자열을 가져옵니다.
            String lastSegment = url.substring(url.lastIndexOf('/') + 1);

            // 문자열 "page=" 다음의 숫자를 가져옵니다.
            String numberString = lastSegment.substring(lastSegment.indexOf('=') + 1);

            // 숫자로 변환하여 출력합니다.
            String number = numberString;

            if (number.equals("0")) url = url.replace("/?page=" + number, "");
            else url = url.replace("/index.shtml/?page=" + number, "/index_" + number + ".shtml");
        }
        this.url = url;
        return url;
    }
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
        ...
        if (dqPageInfo.getParentUrl() == null) {
            ...
            urlList = document.select("div[class=sec_left]").select("ul").select("li");
            if(url.contains("http://www.ce.cn/xwzx/xinwen")){
                domain="http://www.ce.cn/xwzx";     //即时要闻
                htmlSrc = commonUtil.getUrlList(urlList, domain).replace("../../", "/");
            }else if(url.contains("http://www.ce.cn/xwzx/gnsz/gdxw/")){
                domain="http://www.ce.cn/xwzx/gnsz/gdxw";  //时政
                htmlSrc = commonUtil.getUrlList(urlList, domain).replace("./", "/");
            }else if(url.contains("http://www.ce.cn/xwzx/shgj/gdxw/")){
                domain="http://www.ce.cn/xwzx/shgj/gdxw";  //社会·法制
                htmlSrc = commonUtil.getUrlList(urlList, domain).replace("./", "/");
            }else if(url.contains("http://www.ce.cn/xwzx/kj/")){
                domain="http://www.ce.cn/xwzx/kj";         //科技·教育
                htmlSrc = commonUtil.getUrlList(urlList, domain).replace("./", "/");
            }
            ...
            title = document.select("h1[id=articleTitle]").text();
            //System.out.println("title = " + title);

            //create date
            datetimeHtml = document.select("span[id=articleTime]").text();
            datetime = datetimeHtml.replace("年", "-").replace("月", "-").replace("日", "");
            //System.out.println("datetime = " + datetime);

            // content
            Element contentHtml = document.getElementById("articleText");
    ...
    public void endExtension(DqPageInfo dqPageInfo) {
        try {
            ...
            System.out.println("=== CeCNExtension End ===");
        }
    }
