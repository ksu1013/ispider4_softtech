 ...
 * @author 김승욱
 */
public class BelferExtension implements Extension, AddonExtension {
...
//        BbsPage pageContent = Configuration.getInstance().getBbsPage(dqPageInfo.getBbsId(), 1, "*");
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
        ...
        if (dqPageInfo.getParentUrl() == null){
            ...
            urlList = document.select("h2[class=title]");
           ...
           // System.out.println("htmlSrc = " + htmlSrc);
            ...
            title=document.select("h1[class=page-title]").text();
            //System.out.println("title = " + title);

            //create date
            datetimeHtml=document.select("div[class=view-publication-author-and-date]").first().select("span[class=pub-date]").text();
            String[] replaceDatetime=datetimeHtml.split("\\|");
            // 입력된 날짜 문자열을 파싱합니다.
            DateTimeFormatter inputFormatter = null;
            //MMMM dd, yyyy
            //MMM. dd, yyyy
            String[] dateCheck=replaceDatetime[1].split(" ");
            if(dateCheck[1].length()==4){
                inputFormatter = DateTimeFormatter.ofPattern(" MMMM dd, yyyy", Locale.ENGLISH);
            }else if(dateCheck[1].length()==5){
                inputFormatter = DateTimeFormatter.ofPattern(" MMMM. dd, yyyy", Locale.ENGLISH);
            }else if(dateCheck[1].length()==3){
                inputFormatter = DateTimeFormatter.ofPattern(" MMM dd, yyyy", Locale.ENGLISH);
            }

            LocalDate localDate = LocalDate.parse(replaceDatetime[1], inputFormatter);

            // 출력 형식으로 변환합니다.
            DateTimeFormatter outputFormatter = DateTimeFormatter.ofPattern("yyyy:MM:dd HH:mm:ss");
            LocalDateTime localDateTime = localDate.atStartOfDay();
            datetime = outputFormatter.format(localDateTime);
            //System.out.println("datetime = " + datetime);

            // content
            Elements contentHtml = document.select("article[id=node-138041]");
            contentHtml.select(".mobile-pub-share, .publication-authors, .recommended-items, .related-topics").remove();
