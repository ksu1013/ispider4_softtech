 ...
 * @author 김승욱
 */
public class DiamondExtension implements Extension, AddonExtension {
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
        ...
        if (dqPageInfo.getParentUrl() == null){
            ...
            urlList = document.select("article[class=article-box main-article-box]");
            ...
            title=document.select("h1[class=article-title]").text();
            //System.out.println("title = " + title);

            //create date
            datetimeHtml=document.select("span[class=article-pub-date]").text();
            datetime=datetimeHtml.replace(".","-");
            //System.out.println("datetime = " + datetime);

            // content
            Elements contentHtml = document.select("div[class=article-body cXenseParse]");
            contentHtml.select("div[class=gpt-dol-article-middle]").remove();
            if(document.html().contains("<ul class=\"pagination\">")){
                Elements pageList= document.select("ul[class=pagination]").select("li");
                htmlSrc=getUrlList(pageList,"https://diamond.jp",2);
                ...
                for (String item : pageURL) {
                    ...
                    Elements subContentHtml=document.select("div[class=article-body cXenseParse]");
                    contentHtml.select("div[class=gpt-dol-article-middle]").remove();
    ...
    public String getUrlList(Elements urlList, String domain,int j) {
        StringBuffer tagList = new StringBuffer();
        Elements aTagUrlList = urlList.select("a[href]");
        Elements iconCheck=urlList.select("div[class=article-info]").select("h3[class=title]");

        if(j==1){
            tagList.append("<!--List Start-->");
            for (int i=0; i<=19; i++) {
                String currentUrl = aTagUrlList.get(i).attr("href");
                String iconCheckSrc= String.valueOf(iconCheck.get(i).toString().contains("<i class="));
                if(iconCheckSrc.equals("false") && tagList.indexOf(String.valueOf(currentUrl)) == -1){
