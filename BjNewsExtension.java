 ...
 * @author 김승욱
 */
public class BjNewsExtension implements Extension, AddonExtension {
...
//        BbsPage pageContent = Configuration.getInstance().getBbsPage(dqPageInfo.getBbsId(), 1, "*");
    ...
    public String changeRequestURL(String url, DqPageInfo dqPageInfo) {
        if(url.contains("/?page=")){
            ...
            url = url.replace("/?page=" + number, "/"+number+".html");
        }
        this.url=url;
        return url;
    }
    ...
    public Map<String, String> addRequestHeader(DqPageInfo dqPageInfo) {
        ...
        map.put("Host", "www.bjnews.com.cn");
        ...
        map.put("Accept", "*/*");
        map.put("Referer", "https://www.bjnews.com.cn");
        return map;
    }
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
        ...
        if (dqPageInfo.getParentUrl() == null){
            ...
            urlList = document.select("div[class=pin_demo]").select("div[class=imgBor]");
...
//            System.out.println("htmlSrc = " + htmlSrc);
            ...
            title=document.select("div[class=content]").select("h1").text();
//            System.out.println("title = " + title);

            //create date
            datetimeHtml=document.select("span[class=timer]").text();
            datetime=datetimeHtml;

            // content
            Elements contentHtml = document.select("article[class=clearfix]").select("div[class=fl article]");
            contentHtml.select("script").remove();
            contentHtml.select("div[class=giveThumbs-up clearfix]").remove();
            contentHtml.select("div[class=articleComment]").remove();
