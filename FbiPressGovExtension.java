 ...
 * @author 김승욱
 */
public class FbiPressGovExtension implements Extension, AddonExtension {
...
//        BbsPage pageContent = Configuration.getInstance().getBbsPage(dqPageInfo.getBbsId(), 1, "*");
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
        ...
        if (dqPageInfo.getParentUrl() == null){
            ...
            Elements urlList = document.select("p[class=title]");
...
//          //create date
            try {
                Elements datacheck  = document.select("div[class=movable removable mosaic-tile mosaic-IDublinCore-effective-tile mosaic-format-notime press-release-date]");

                if(datacheck.toString().equals("")){
                    datetimeHtml=document.select("div[class=field-formatter--datetime-default field_date]").select("time").attr("datetime");
                    inputFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssXXX");
                }else{
                    datetimeHtml=document.select("div[class=movable removable mosaic-tile mosaic-IDublinCore-effective-tile mosaic-format-notime press-release-date]").text();
            ...
            // content
            //Elements contentHtml = document.select("div[class=mosaic-grid-row row]");
            Elements contentHtml = document.select("div[class=node-body]");
            if(contentHtml.toString().equals("")){
                contentHtml = document.select("div[class=mosaic-grid-row row]");
                contentHtml.select("div[class=mosaic-grid-cell mosaic-position-leftmost mosaic-width-two-thirds col-md-8]").remove();
                contentHtml.select("div[class=movable removable mosaic-tile mosaic-fbigov.theme.relatedresources-tile]").remove();
            }else{
                contentHtml = document.select("div[class=node-body]");
            }
