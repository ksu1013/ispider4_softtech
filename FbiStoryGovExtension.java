 ...
 * @author 김승욱
 */
public class FbiStoryGovExtension implements Extension, AddonExtension {
...
//        BbsPage pageContent = Configuration.getInstance().getBbsPage(dqPageInfo.getBbsId(), 1, "*");
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
        ...
        if (dqPageInfo.getParentUrl() == null){
            ...
            //title
            title = document.select("h1[class=sr-only]").text();
            //System.out.println("title = " + title);

//          //create date
            try {
                String datetimeHtml = document.select("div[class='']").first().text();
            ...
            // content
            Elements contentHtml = document.select("div[class=mosaic-tile-content]");
            contentHtml.select("p[class=sharing-button]").remove();
            contentHtml.select("div[class=news-article-centered dp-large]").remove();
            contentHtml.select("div[class=featured-content featured-content-backgroundimage\n" +
                    "                portal-type-story landscape]").remove();
            contentHtml.select("div[class=castle-tile-wrapper]").last().remove();
