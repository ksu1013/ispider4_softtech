 ...
 * @author 김승욱
 */
public class CIAGovExtension implements Extension, AddonExtension {
    ...
    public void startExtension(DqPageInfo dqPageInfo, String homePath) {
        System.out.println("=== CIAGovExtension Start ===");
...
//        BbsPage pageContent = Configuration.getInstance().getBbsPage(dqPageInfo.getBbsId(), 1, "*");
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
        ...
        if (dqPageInfo.getParentUrl() == null){
            ...
            Elements urlList = document.select("a.d-block.h-100");
            ...
            //title
            title = document.select("h1.hero-title").text();

            //create date
            String datetimeHtml = document.selectFirst("div.hero-content p").text();
            LocalDate date = LocalDate.parse(datetimeHtml, DateTimeFormatter.ofPattern("MMMM d, yyyy", Locale.ENGLISH));
            datetime = date.format(DateTimeFormatter.ofPattern("yyyy-MM-dd 00:00:00"));


            // content
            Elements contentHtml = document.select(".row ").select(".article-content");
            // source 태그 제거
            contentHtml.select("source").remove();

            //이미지 src 에 도메인 추가
            Elements imgs = contentHtml.select("img");
            for (Element img : imgs) {
                String src = img.attr("src");
                if (src != null && !src.contains("static")) {
                    img.remove();
                }
                String absoluteSrc = "https://www.cia.gov" + src;
                img.attr("src", absoluteSrc);
            }



            //이미지에 src 제외 하고 나머지 지우기
            contentHtml.select("img").removeAttr("srcset")
                    .removeAttr("sizes").removeAttr("style")
                    .removeAttr("alt").removeAttr("loading");
    ...
    public void endExtension(DqPageInfo dqPageInfo) {
        try {
            ...
            System.out.println("=== CIAGovExtension End ===");
        }
    }
