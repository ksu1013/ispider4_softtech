 ...
 * @author 김승욱
 */
public class FranceExtension_bak implements Extension, AddonExtension {
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
        ...
        if (dqPageInfo.getParentUrl() == null){
            ...
            dateList=document.select("select[name=fichier]");
            dateList.select("option").first().remove();
            htmlSrc=getUrlList(dateList,urlList,"https://basedoc.diplomatie.gouv.fr/vues/Kiosque/FranceDiplomatie/kiosque.php?");
    ...
    public String getUrlList(Elements dataList, Elements urlList, String domain) {
        StringBuffer tagList = new StringBuffer();
        urlList = urlList.select("a[href]");
        dataList = dataList.select("option[value]");

        tagList.append("<!--List Start-->");
        for (Element link : urlList) {
            String currentUrl = link.attr("href");
            String currentdate = dataList.attr("value");
            if(!currentUrl.contains(".pdf")){
                tagList.append("\n<a href =\"" + domain+currentdate+ currentUrl + "\"></a>");
            }
        }
        tagList.append("\n<!--List End-->");
