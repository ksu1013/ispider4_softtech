 ...
 * @author 김승욱
 */
public class DiaSaTGovExtension implements Extension, AddonExtension {
...
//        BbsPage pageContent = Configuration.getInstance().getBbsPage(dqPageInfo.getBbsId(), 1, "*");
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
        ...
        if (dqPageInfo.getParentUrl() == null){
            ...
            String urlList = document.select("item")
                    .select("guid[ispermalink=false]")
                    .toString()
                    .replace("<guid ispermalink=\"false\">","\n<a href =\"")
                    .replace("</guid>","\"></a>");
            tagList.append("<!--List Start-->");
            tagList.append(urlList);
            tagList.append("\n<!--List End-->");
...
//          //create date
            try {
                String datetimeHtml = document.select("div[class=category-date]").text();
                datetimeHtml =datetimeHtml.replace("Sept","September");
