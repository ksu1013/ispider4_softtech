 ...
 * @author 김승욱
 */
public class FsbRuExtension implements Extension, AddonExtension {
...
//        BbsPage pageContent = Configuration.getInstance().getBbsPage(dqPageInfo.getBbsId(), 1, "*");
    ...
    public String changeRequestURL(String url, DqPageInfo dqPageInfo) {
        //http://www.fsb.ru/fsb/comment/remark.htm?f=1&blk=10438552.html
        //http://www.fsb.ru/fsb/comment/remark.htm!f=1&blk=10438552.html
        //http://www.fsb.ru/fsb/comment/remark.htm!f=26&blk=10438552.html

        //http://www.fsb.rufsb/comment/remark/single.htm!id=10438244@fsbComment.html
        //http://www.fsb.ru/fsb/comment/remark/single.htm%21id%3D10438244%40fsbComment.html
        //http://www.fsb.ru/fsb/comment/remark/single.htm%21id%3D10438244%40fsbComment.html"
        String urlEncodingReady=null;
        if(url.contains("blk")) {
            urlEncodingReady = url.replace("http://www.fsb.ru/fsb/comment/remark.htm?f", "!f");
            String encodedUrl = null;
            try {
                System.out.println("urlEncodingReady = " + urlEncodingReady);
                encodedUrl = URLEncoder.encode(urlEncodingReady, "UTF-8");
                url = "http://www.fsb.ru/fsb/comment/remark.htm" + encodedUrl;
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
        ...
        if (dqPageInfo.getParentUrl() == null){
            ...
            urlList = document.select("li[class=item]");
            htmlSrc=commonUtil.getUrlList(urlList,"http://www.fsb.ru/");
            ...
            title=document.select("div[class=_attr _attr_title form-group title type0]").text();
            System.out.println("title = " + title);

            //create date
            datetimeHtml=document.select("div[class=_attr _attr_date form-group date type5]").text();

            // 주어진 문자열을 LocalDate 객체로 파싱 (주의: 형식 지정 필요)
            DateTimeFormatter originalFormatter = DateTimeFormatter.ofPattern("dd.MM.yyyy");
            LocalDate localDate = LocalDate.parse(datetimeHtml, originalFormatter);

            // 원하는 포맷으로 변환
            DateTimeFormatter targetFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
            datetime = localDate.atStartOfDay().format(targetFormatter);


            // content
            Elements contentHtml = document.select("div[class=_attr _attr_text form-group common type1]");
