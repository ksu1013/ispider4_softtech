 ...
 * @author 김승욱
 */
public class FasExtension implements Extension, AddonExtension {
...
//        BbsPage pageContent = Configuration.getInstance().getBbsPage(dqPageInfo.getBbsId(), 1, "*");
    ...
    public String changeRequestURL(String url, DqPageInfo dqPageInfo) {
        if(url.contains("/?page=")){
            ...
            url=url.replace("/?page="+number,"/pages/"+number+"/");
        }
        this.url=url;
        return url;
    }
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
        ...
        if (dqPageInfo.getParentUrl() == null){
            ...
            urlList = document.select("div[class=posts-archive__grid]");
            ...
            title=document.select("h1[class=single-post__title-area-title]").text();
            //System.out.println("title = " + title);

            //create date
            try {
                datetimeHtml=document.select("div[class=single-post__title-area-data]").select("span").first().text();
                // 입력 날짜 형식 지정
                ...
                Date inputDate = inputFormat.parse(datetimeHtml);
            ...
            // content
            Elements contentHtml = document.select("div[class=single-post block-content-grid]");
            contentHtml.select("header[class=single-post__header block-content-grid__header]").remove();
            contentHtml.select("section[class=block block--full-width faqs]").remove();
            contentHtml.select("section[class=block block--full-width content-listing content-listing--post-type-publications]").remove();
