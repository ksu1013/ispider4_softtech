 ...
 * @title 안후이 뉴스 네트워크 Extension (PXAHNEWS)
 * @since 2023-09-27
 */
public class AHNewsExtension implements Extension, AddonExtension {
    /* 테스트용 S */
    ...
    public String changeRequestURL(String url, DqPageInfo dqPageInfo) {
        ...
        if (matcher.find()) {
            ...
            if(!page.equals("") && !page.equals("1")){
                url = url.replace("node_562.html", "node_562_" + page + ".html");
            }
        }
        if (url.contains("?")) {
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
           ...
           if(dqPageInfo.getParentUrl() == null) {    // 0depth
               Elements urlList = document.select(".dataList .clearfix h3");
               ...
               /* 파싱 start */
               //title
               title = document.select(".h-title").text();

               //create date
               datetime = document.select(".h-time").text();

               //content
               Elements contentHtml = document.select("#p-detail");
               contentHtml.select("script, .p-tags").remove();
