 ...
 * @author 강태훈
 */
public class FJSENExtension implements Extension, AddonExtension {
    /* 테스트용 S */
    ...
    public String changeRequestURL(String url, DqPageInfo dqPageInfo) {
        System.out.println("changeRequestURL input url = " + url);
        ...
        if (matcher.find()) {
            ...
            System.out.println("matcher = " + page);
            if(!page.equals("") && !page.equals("1")){
                url = url.replace("fj_tw.htm", "fj_tw_" + page + ".htm");
            }
        }

        if (url.contains("?")) {
            url = commonUtil.getFullDomain(url);
        }
        System.out.println("바뀐 URL = " + url);
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
           ...
           if(dqPageInfo.getParentUrl() == null) {    // 0depth
               Elements urlList = document.select(".cont-left .list_page");
               ...
               /* 파싱 start */
               //title
               title = document.select(".cont_head h1").text();

               //create date
               datetime = document.select("#pubtime_baidu").text();

               //content
               Elements contentHtml = document.select("#new_message_id");
               commonUtil.removeComments(contentHtml);    // 주석 제거
