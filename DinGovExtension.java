 ...
 * @author 김승욱
 */
public class DinGovExtension implements Extension, AddonExtension {
...
//        BbsPage pageContent = Configuration.getInstance().getBbsPage(dqPageInfo.getBbsId(), 1, "*");
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
        ...
        if (dqPageInfo.getParentUrl() == null){
            ...
            Elements urlList = document.select("h3.catItemTitle").select("a");
            ...
            //create date
            String datetimeHtml = document.select(".itemDateCreated").text();
            // 입력 형식 지정
            DateTimeFormatter inputFormatter = DateTimeFormatter.ofPattern("EEEE, d MMMM yyyy HH:mm", Locale.ENGLISH);

            // 출력 형식 지정
            DateTimeFormatter outputFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");

            // 문자열을 LocalDateTime으로 파싱
            LocalDateTime dateTime = LocalDateTime.parse(datetimeHtml, inputFormatter);

            // 지정된 형식으로 변환
            datetime = dateTime.format(outputFormatter);

            //System.out.println("outputDate = " + outputDate);


            // content
            Elements contentHtml = document.select(".itemFullText");
            contentHtml.select("p:matches(^(?:&nbsp;|\\s)*$)").remove();
            contentHtml.select("p[style]").remove();
