 ...
 *
 */
public class CHouseExtension implements Extension, AddonExtension {
    ...
    public static final String MAIN_URL = "https://www.chathamhouse.org";

    private Random random;
    private static int MIN_DELAY = 7000; // 7초
    private static int MAX_DELAY = 10000; // 10초
    ...
    public String changeHtml(String htmlSrc, DqPageInfo dqPageInfo) {
           ...
           try {
               ...
               driver.get(url);
               int delay = random.nextInt(MAX_DELAY - MIN_DELAY + 1) + MIN_DELAY; // 3초에서 5초까지의 랜덤한 시간
               try {
               ...
               if (dqPageInfo.getParentUrl() == null) {     // 0depth
                   Elements urlList = document.select(".views-element-container .l-grid__item");
                   String DUMMY_HOST = "10.10.10.214";
                   htmlSrc = commonUtil.getUrlList(urlList, "http://" + DUMMY_HOST + ":8080/dummy?");
//                   System.out.println("htmlSrc = " + htmlSrc);
                   ...
                   /* 파싱 start */
                   //title
                   title = document.select(".person-bio__title").text();

                   //create date
                   String datetimeHtml = document.getElementsByAttributeValue("property", "article:modified_time").attr("content");
                   DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ssZ");
                   LocalDateTime localtime = LocalDateTime.from(formatter.parse(datetimeHtml));
                   datetime = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss").format(localtime);

                   //content
                   //메인 이미지
                   Elements leadImages = document.select(".person-bio");
                   document.select("noscript").remove();

                   if(leadImages.size() > 0){
                       String imgSrc = leadImages.select("img").attr("data-srcset");
                       content = "<img src=\"" + MAIN_URL + imgSrc + "\">\n";
                       content = commonUtil.removeImgParam(content);
                   }
                   // 내용
                   Elements contentHtml = document.select(".content-layout__main");
                   contentHtml.select(".person-bio, button").remove();
